...
from prompts.rag_prompts import fda_cybersecurity_rag_prompt_template
...
def fda_cybersecurity_rag(docs: list[Document]):
    ...
    store = LocalFileStore("./cybersecurity-cache/")
    ...
    vectorstore = Qdrant.from_documents(
        ...
        cached_embedder,
        location=":memory:",
        collection_name="fda_cybersecurity_guidance_for_samd_and_aiml",
    )

    # set the llm model
    fda_cybersecurity_llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
    ...
    fda_cybersecurity_rag_chain = (
        RunnableLambda(retrieve_and_format)
        | RunnablePassthrough.assign(
            answer=fda_cybersecurity_rag_prompt_template
            | fda_cybersecurity_llm
            | StrOutputParser()
        )
        | RunnableLambda(
            lambda x: {
    ...
    return fda_cybersecurity_rag_chain
...
def create_redis_semantic_cache(embedding_model):
    try:
        ...
        semantic_cache = RedisSemanticCache(
            ...
            name="fda_cybersecurity_rag_cache",
            prefix="fda_cybersecurity_rag_cache",
        )
...
if __name__ == "__main__":
    ...
    # Create the chain directly
    chain = fda_cybersecurity_rag(docs)
    response = chain.invoke(
        "Does a SOUP document for SaMD for Class II medical device lists CVEs for each library or should it list CWEs instead?"
    )
    print(response)
